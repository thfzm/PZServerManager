...
public partial class SandboxTab : UserControl
{
    ...
    private sealed class ChoiceItem
    {
        ...
        public ChoiceItem(SandboxChoice c) { Choice = c; }
    ...
    private void BuildCategoryTabs()
    {
        ...
        foreach (var cat in SandboxSchema.CategoryOrder)
        ...
        switch (def.Type)
        {
            ...
            case SandboxFieldType.IntChoices:
            {
                ...
                if (def.Choices is not null)
                    foreach (var c in def.Choices) combo.Items.Add(new ChoiceItem(c));
                ...
                setter = v =>
                {
                    ...
                    for (int i = 0; i < combo.Items.Count; i++)
                        if (combo.Items[i] is ChoiceItem ci && ci.Choice.Value == n)
    ...
    private TabPage BuildRawPage()
    {
        ...
        {
            Dock = DockStyle.Top,
            Text = "스키마에 없는 sandbox 키들. 값을 그대로 보존합니다.",
    ...
    private void LoadCurrent()
    {
        ...
        try
        {
            ...
            {
                if (_lua.TryGet(key, out var v)) ed.Set(v);
    ...
    private void OnSave(object? sender, EventArgs e)
    {
        ...
        foreach (var (path, ed) in _editors) _lua.Set(path, ed.Get());
        foreach (var (path, tb) in _rawEditors) _lua.Set(path, tb.Text);
    ...
    private void OnProfileChanged(object? sender, EventArgs e)
    {
        ...
        if (_dirty)
        {
            var r = MessageBox.Show(this,
                $"'{_profile.Name}_SandboxVars.lua' 의 변경사항을 버리고 전환할까요?",
