...
public sealed class ModScanner
{
    ...
    /// One workshop item can ship multiple mods, so we yield each mod separately.
    public List<LocalMod> Scan()
    {
        ...
        {
            var workshopName = Path.GetFileName(workshopDir);
            if (!long.TryParse(workshopName, out var id)) continue;
