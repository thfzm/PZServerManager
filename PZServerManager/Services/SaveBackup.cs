...
public sealed class SaveBackup
{
    ...
    public List<string> ListBackups(string saveName) => ListBackupsIn(saveName, ResolveBackupDir());
    ...
    public async Task<string> BackupAsync(string saveName, IProgress<string>? log, CancellationToken ct)
    {
        ...
        if (!Directory.Exists(src)) throw new DirectoryNotFoundException($"세이브 '{saveName}' 가 존재하지 않습니다.");
    ...
    {
        if (!File.Exists(zipPath)) throw new FileNotFoundException("백업 파일 없음.", zipPath);
