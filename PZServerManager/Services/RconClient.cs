...
/// Handles multi-packet responses by sending an empty SERVERDATA_RESPONSE_VALUE after each
/// EXECCOMMAND and accumulating bodies until the server echoes the empty packet back —
/// this is the canonical Valve trick for detecting end-of-response.
public sealed class RconClient : IDisposable
{
    ...
    public async Task ConnectAsync(string host, int port, string password, TimeSpan timeout, CancellationToken ct)
    {
        ...
        // The server may send an empty RESPONSE_VALUE before AUTH_RESPONSE.
        while (true)
        {
            ...
            if (type == SERVERDATA_AUTH_RESPONSE)
            {
                ...
                {
                    Disconnect();
                    throw new UnauthorizedAccessException("RCON authentication failed (wrong password).");
                }
